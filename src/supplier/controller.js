...
export const getSupplier = async (req, res) => {
   try {
      ...
      res.send({ status: true, data: Supplier })
   } catch (err) {
...
export const updateSupplier = async (req, res) => {
   try {
      ...
      if (!isValidObjectId(id))
         return res.status(400).json({ status: false, message: "invalid supplier Id" });
...
export const deleteSupplier = async (req, res) => {
   try {
      ...
      const deleteSuppl = await Customer.findOneAndDelete({ _id: id }, { new: true });
      if (!deleteSuppl)
         return res
            .status(400)
